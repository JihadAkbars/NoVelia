...
export const AdminStoryEdit: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clean story payload (remove ID for inserts)
      ...
      alert('Error saving story: ' + (error.message || 'Check if you have created the tables in Supabase. See console for details.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAiImprove = async (index: number) => {
    ...
    try {
      ...
      alert("AI Service error. Please check your API key.");
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="flex items-center mb-8">
        <button onClick={() => navigate(-1)} className="mr-4 p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7 7-7" />
          </svg>
        </button>
        <h1 className="text-3xl font-black tracking-tight">{isEdit ? 'Edit Story' : 'New Story'}</h1>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-10">
        <section className="bg-white dark:bg-zinc-900 p-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Synopsis</label>
            <textarea 
              rows={4}
              className="w-full px-4 py-3 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 outline-none focus:ring-2 focus:ring-amber-500 transition-all"
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Author Bio</label>
            <textarea 
              rows={2}
              className="w-full px-4 py-3 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 outline-none focus:ring-2 focus:ring-amber-500 transition-all"
        ...
        <section className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <button 
              type="button" 
              onClick={addChapter}
              className="text-amber-500 font-bold hover:text-amber-600 flex items-center transition-colors"
            >
              ...
              <div key={idx} className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm relative group">
                <div className="flex justify-between items-center mb-4">
                   ...
                   <button 
                    ...
                    className="text-red-500 hover:text-red-600 p-1 md:opacity-0 group-hover:opacity-100 transition-all"
                   >
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    <textarea 
                      ...
                      className="w-full px-4 py-2 rounded-lg bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 outline-none font-serif text-lg leading-relaxed"
                      value={ch.content}
                    ...
                    <button
                      ...
                      className="absolute bottom-4 right-4 bg-amber-500 text-white p-3 rounded-full shadow-lg hover:scale-110 active:scale-95 transition-all flex items-center justify-center disabled:opacity-50 z-10"
        ...
        <div className="sticky bottom-8 py-4 px-6 bg-white/90 dark:bg-zinc-950/90 backdrop-blur border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-2xl flex justify-end items-center gap-4 z-40">
           <button 
            type="button" 
            onClick={() => navigate('/admin/dashboard')}
            className="px-6 py-2 font-bold text-zinc-500 hover:text-zinc-700 transition-colors"
           >
             Cancel
           </button>
           <button 
            type="submit"
            disabled={loading}
            className="px-10 py-3 bg-amber-500 text-white font-bold rounded-2xl hover:bg-amber-600 shadow-lg shadow-amber-500/20 disabled:opacity-50 transition-all flex items-center"
           >
             {loading ? (
               <>
